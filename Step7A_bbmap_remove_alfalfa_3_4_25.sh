...
#SBATCH --cpus-per-task=55  				##request number of cpus 
...
#SBATCH --output="sb%j-bbmap-alfalfa_removal"			##stores slurm logfile to text file called slurmlog_(assigned job number) 

module load bbmap
module load samtools

#To start, this is set up for use with trimmed input fastq files ending in R1_001_trim.fastq and R2_001_trim.fastq corresponding to forward and reverse reads, respectively
...
for file in *R1_001_trim.fastq; do
		...
		bbmap.sh threads=55 minid=0.90 in1="$shortname"R1_001_trim.fastq in2="$shortname"R2_001_trim.fastq ref=Alfalfa_genome.fasta nodisk out="$shortname"_alfalfa_bbmap90_out.bam
		samtools view --threads 55 -b -f 13 "$shortname"_alfalfa_bbmap90_out.bam > "$shortname"_unmapped.bam
done
...
for bam in *_alfalfa_bbmap_90_out.bam; do
	echo $bam "unmapped reads"
	samtools view --threads 55 -c -f 4 $bam
...
for fastq in *_interlaced.fastq; do
	...
	reformat.sh in="$fastq" out1="$shortname"_no_alfalfa_1.fastq out2="$shortname"_no_alfalfa_2.fastq
done
